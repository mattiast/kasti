...
readPositions conn = do
  (rows :: [(String, EpisodeId) :. Episode :. ProgressMsg]) <-
    query_
      conn
...
writeEpisodes fid eps conn = withTransaction conn $ do
  ...
  numRows <-
    executeMany
      conn
      "insert into episodes(feed_id, url, title, date) values (?,?,?,?) on conflict do nothing"
...
writePosition msg conn =
  withTransaction conn $
    void $
      execute
        conn
        "insert into progress(episode_id, position, duration) values (?, ?, ?)\
        \ on conflict(episode_id) do update set position = ?, duration = ?"
        (msg :. (prPos msg, prDuration msg))
